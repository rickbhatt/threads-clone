  ...
  handler: async (ctx, args) => {
    ...
    if (args.threadId) {
      ...
      if (originalThread?.userId) {
        ...
        await ctx.scheduler.runAfter(200, internal.push.sendPushNotification, {
          pushToken,
          messageTitle: `${user?.first_name} commented on your post`,
...
export const likeThread = mutation({
  ...
  handler: async (ctx, args) => {
    let currentUser = await getCurrentUserOrThrow(ctx);
    ...
    const updatedThread = await ctx.db.get(args.messageId);
    if (updatedThread?.userId) {
      const user = await ctx.db.get(updatedThread?.userId);
      const pushToken = user?.pushToken;
      ...
      await ctx.scheduler.runAfter(200, internal.push.sendPushNotification, {
        pushToken,
        messageTitle: "New Like",
        messageBody: `${currentUser?.first_name} liked your thread`,
